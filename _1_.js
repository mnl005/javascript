...
// 자바스크립트는 컴파일 수행하지 않는 인터프리터 언어로 코드가 한 줄씩 순차적으로 실행
// 자바스크립트는 메모리를 개발자가 직접 관리하지 않음으로 언매니지드 언어이다
// 자바스크립트 표준 사양 : ECMA-262
// npm : 자바스크립트 패키지 매니저


// 변수 ----------------------------------------------------------------------------------------
// 10 + 20 ----> 연산을 위해 리터럴(10),연산자(+),표현식(10 + 20),의미해석(파싱)
// 연산을 수행시 좌우변의 숫자값(피연산자) 를 1바이트 크기의 메모리셀에 저장 또는 읽어온다
// 컴퓨터는 모든 데이터를 2진수로 저장
// 계산 결과도 메모리에 저장되나 결과값이 저장된 메모리 주소로 접근할 경우 오류 발생 가능성 높다
// 자바스크립트는 직접적인 메모리 제어를 허용하지 않는다
// 변수 : 값을 저장하기 위해 확보한 메모리 공간자체나 메모리 공간 식별위해 변수 사용(값의 위치를 가리키는 상징적 이름)
// 자바스크립트는 변수를 통해 값이 저장된 메모리 주소를 안전하게 접근
...
// 변수 호이스팅 : 변수 선언문이 코드의 선두로 끌어 올려진 것처럼 동작하는 자바스크립트의 고유의 특징, 단 값의 할당은 런타임 시점에 실행
...
// 문(명령문) : 프로그램을 구성하는 기본 단위이자 최소 실행 단위, 문의 집합 = 프로그램, 문을 작성하고 순서에 맞게 나열하는 것 = 프로그래밍
// 문은 토큰으로 구성
...
// 문은 표현식인 문과 표현식이 아닌 문 두가지 존재, 변수에 할당 가능하면 표현식인 문 이다
// 변수 선언문은 값으로 평가 불가함으로 표현식이 아닌 문
let v18;
...
// 메모리의 값을 참조하는 과정 : 식별자 통해 값이 저장된 메모리 공간의 주소를 찾는다 -> 메모리 셀의 바이트 수를 확인 -> 값을 데이터 타입 따라 다르게 해석
...
console.log(typeof undefined_); // undefined ----> 선언한 적 없는 식별자는 타입 확인시 undefined
...
// 블록문 : 0개 이상의 문을 중괄호로 묶은것, 블록문 끝에는 세미콜론 붙이지 않는다
...
// 명시적 타입 변환, 타입 캐스팅
let vv14 = (10).toString();
console.log(typeof vv14); // string, 원본에 영향주지 않는다

// 암묵적 타입 변환, 타입 강제 변환, 새로운 타입의 값을 만들어 단 한 번 사용후 버린다
console.log(typeof (10 + ''));
console.log("암묵적 타입 변환 예시");
console.log('10' + 2);
console.log(5 * '10');
console.log(!0);
if(1){console.log("if 문 타입 변환")};
console.log(`1 + 1 = ${1 + 1}`);
